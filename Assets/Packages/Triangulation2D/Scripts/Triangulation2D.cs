...
namespace mattatz.Triangulation2DSystem {
	...
	public class Triangulation2D {
		...
		public List<Vertex2D> V = new List<Vertex2D>(); // vertices in PSLG
		public List<Segment2D> S = new List<Segment2D>(); // segments in PSLG

		public List<Vertex2D> P = new List<Vertex2D>(); // vertices in DT
		public List<Segment2D> E = new List<Segment2D>(); // segments in DT

		Polygon2D PSLG;
		public List<Triangle2D> T = new List<Triangle2D>();
		...
		public Mesh Build (List<Vector2> range) {
			var mesh = new Mesh();
			...
			float quadSize_x = range[3].x - range[0].x;
			float quadSize_y = range[1].y - range[0].y;
			
			for (int i = 0; i < vertices.Length; i++)
			{
				normals[i] = new Vector3(0f, 0f, 1f);
				float x_coord = vertices[i].x - range[0].x;
				float y_coord = vertices[i].y - range[0].y;
				uvs[i] = new Vector2(Mathf.Abs(x_coord / quadSize_x), Mathf.Abs(y_coord / quadSize_y));
			}
